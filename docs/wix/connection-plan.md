...
| 観点 | 旧方針（採用しない） | 新方針（現行） |
|---|---|---|
| **母体リポジトリ** | 既存リポジトリ（例: `ryoochi-wix-site`）を母体にする | Wix Studio GitHub Integration が生成したリポジトリを母体にする |
| **src/ の調達** | 既存 Repo に `src/` をコピーして持ち込む | Wix が自動生成した `src/` をそのまま維持する |
| **運用資産の方向** | Wix 構造を既存 Repo に合わせて持ち込む | CI・docs・agents 等を Wix 生成 Repo に移植する |
| **CI コマンド** | `wix publish --approve-preview`（本番公開を想定） | `wix preview --source remote`（プレビュー URL 生成のみ） |
| **連携の仕組み** | Wix 側が認識しないリポジトリへ接続しようとした | Wix が認識しているリポジトリで `--source remote` が正しく機能する |
...
**「既存リポジトリに Wix 構造を合わせる」ではなく「Wix 生成リポジトリに既存運用資産を載せる」が現行の主線である。**
今後この方向を逆に戻す変更（既存 Repo を母体に戻す、`src/` をコピーで差し替える）は採用しない。
