...
### NEXT7-00 Phase7 Execution Layer Boundary (SoT)
- Phase7 は **Phase6 完了後にのみ** 着手する。
- Phase7 は **作成・変更実行レイヤー** として固定し、対象を次の 4 領域に限定する。
  1. write-plan
  2. execution plan
  3. confirm付き変更実行補助
  4. Figma / GitHub / AI / Run の変更連携
- Phase7 は、Phase5 の利用支援責務と Phase6 の管理統制責務の上に、実行責務を分離配置するフェーズとして扱う。
- Hubは成果物SoTではない。Hub は **orchestration layer** として `thread / run / plan / audit` を保持し、成果物の正本は GitHub / Figma / Drive 側に残す。
- Phase7 の対象は次に固定する。
  - change request から `write-plan` と `execution plan` を生成し、対象リソース・前提条件・confirm 条件・期待結果を明示すること
  - confirm を伴う変更実行フローを整備し、無確認実行ではなく operator confirm を経由して GitHub / Figma / AI / Run 連携を進めること
  - execution job により read / validate / prepare / execute / record を追跡し、Hub 側には run と audit を残すこと
  - 実行結果の記録を Hub に集約しつつ、成果物の更新先は GitHub / Figma / Drive の正本へ戻すこと
- Phase7 の主要成果物は次に固定する。
  - execution plan
  - confirm flow
  - execution job
  - audit
...
- 上記の対象外は、Phase7 の完了条件や UI / API / orchestration / 運用判断へ混入させない。
- Phase7 の詳細タスクは backlog (`backlog/phase7-execution-layer.md`) で管理し、本節は境界と対象のみを SoT とする。

### NEXT7-01 Phase7 Completion Criteria (SoT)
Phase7 の完了は、単に変更ボタンが増えた状態ではなく、**confirm 付き実行運用が一貫して成立した状態**に到達したときにのみ成立する。

1. SoT / Boundary 固定
- Phase7 の境界、Hub の位置づけ、execution plan と confirm flow の責務が SoT として固定されていること。
- 少なくとも次を正として参照できること。
  - `docs/ai/core/workflow.md`
- `backlog/phase7-execution-layer.md`
- Hubは成果物SoTではない、という原則が文書・UI・運用説明で一貫していること。

2. Execution Plan 固定
- `write-plan` と `execution plan` が分離され、対象、差分意図、前提条件、confirm 条件、実行後の記録先を保持できること。
- plan が GitHub / Figma / Drive のどこを更新するのかを明示し、Hub 内部の plan だけを成果物正本として扱わないこと。

3. Confirm Flow 固定
- 変更実行は必ず confirm を通過し、confirm なし自動実行へ退化しないこと。
- confirm UI / API / job state が `requested / confirmed / rejected / executed / recorded` などの状態遷移で追跡できること。
- Phase5 Workspace の補助導線や Phase6 Admin の管理導線に、自律実行判断を逆流させないこと。

4. Execution Job 固定
- execution job が `plan -> confirm -> execute -> record -> audit` を追跡できること。
- GitHub / Figma / AI / Run の変更連携が job 単位で紐付き、失敗時は `failure_code` と再試行方針を残せること。
- 実行 job は Hub の orchestration layer に属し、成果物正本の置き場と混線しないこと。

5. Run Integration 固定
- run detail から `write-plan / execution plan / execution job` を相互参照できること。
- run と plan/job の関連が Hub の orchestration 記録として追跡でき、成果物正本との境界を壊さないこと。

6. Audit 固定
- audit により、誰が何を確認し、どの plan をどこへ実行し、どの成果物が更新されたか追跡できること。
- `plan.created / plan.approved / plan.rejected / job.created / job.started / job.finished` を主要イベントとして追跡できること。
- draft と committed の確定条件が文書と実装で一致していること。

7. selftest 固定
- Phase7 の主要ケースが `scripts/selftest.js` に統合登録され、継続実行されること。
- 少なくとも次を selftest で確認できること。
  - execution plan
  - confirm flow
  - execution job
  ...
  - audit
  - ops console
  - Hub orchestration boundary

8. Runbook 固定
- Phase7 の運用 runbook が存在し、少なくとも次を文書化していること。
  - confirm待ち
  - 承認
  - 却下
  - 実行失敗時対応
  - rollback
  - audit 確認
  - GitHub / Figma / Drive への正本反映確認
  - Hub 側 record 確認
- runbook は、confirmなし自動実行や完全自律エージェントを Phase7 の運用前提にしないこと。

9. Ops Console 固定
- ops console で少なくとも `plan status` `confirm status` `execution job status` `target system` `failure summary` を確認できること。
